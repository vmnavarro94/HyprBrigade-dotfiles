...
HyprBrigade_DIR="$HOME/HyprBrigade"

# exit if cannot find local version
if [ -z "$local_version" ]; then
  notify-send -i "$iDIR/error.png" 'ERROR !?!?!!' "Unable to find HyprBrigade version. Exiting."
  exit 1
fi

# GitHub URL - HyprBrigade
branch="main"
github_url="https://github.com/vmnavarro94/HyprBrigade/tree/$branch/config/hypr/"
...
# Fetch the version from GitHub URL - HyprBrigade
github_version=$(curl -fsSL -A "Mozilla/5.0" "$github_url" | grep -o 'v[0-9]\+\.[0-9]\+\.[0-9]\+' | sort -V | tail -n 1 | sed 's/v//')

# Cant find  GitHub URL - HyprBrigade version
if [ -z "$github_version" ]; then
  notify-send -i "$iDIR/error.png" 'HyprBrigade:' "Unable to determine GitHub version."
  exit 1
fi

# Comparing local and github versions
if [ "$(echo -e "$github_version\n$local_version" | sort -V | head -n 1)" = "$github_version" ]; then
   notify-send -i "$iDIR/note.png" "HyprBrigade:" "No update available"
  exit 0
else
  # update available
  ...
  response=$($notify_cmd_shot "HyprBrigade:" "Update available! Update now?")

  case "$response" in
    "action1")  
      if [ -d "$HyprBrigade_DIR" ]; then
        ...
        kitty -e bash -c "
          git clone --depth=1 https://github.com/vmnavarro94/HyprBrigade.git $HyprBrigade_DIR &&
          cd \"$HyprBrigade_DIR\" &&
          chmod +x copy.sh &&
          ./copy.sh &&
		  notify-send -u critical -i "$iDIR/ja.png" 'Update Completed:' 'Kindly log out and relogin to take effect'
